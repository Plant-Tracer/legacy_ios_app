...
Filters.convolve = function(pixels, weights, opaque) {
  ...
  for (var y=0; y<h; y++) {
    for (var x=0; x<w; x++) {
      ...
      saveData[dstOff+1] = g;
      saveData[dstOff+2] = b;
      saveData[dstOff+3] = a + alphaFac*(255-a);
  ...
  return this.laplaceThreshould(output,saveData,50);
};

Filters.laplaceThreshould = function(pixels,saveData,threshould){
  var src = pixels.data;
  var sw = pixels.width;
  var sh = pixels.height;
  ...
  for(var i = 0; i < sh;i++){
    for (var j = 0; j < sw; j++) {
      ...
      var bottom;
      left = j < 1 ? 0 : j - 1;
      right = j == sw -1 ? sw - 1 : j+1;
      top = i < 1 ? 0 : i -1;
      bottom = i == sh - 1 ? sh - 1 : i +1; 

      var updown = Math.abs(test[i][left] - test[i][right]);
      var leftRight = Math.abs(test[top][j] - test[bottom][j]);

      saveDiff[saveDiff.length] = updown;
      saveDiff[saveDiff.length] = leftRight;

    }
  }
  saveDiff.sort(function(a, b){return a - b});
  threshould = saveDiff[Math.round(saveDiff.length * 0.90)];
  ...
  for(var i = 0; i < sh;i++){
    ...
    if((test[i][left] < 0 && test[i][right] > 0) || (test[i][left] > 0 && test[i][right] <0)||(test[top][j] < 0 && test[bottom][j] >0) || (test[top][j] > 0 && test[bottom][j] < 0)){
      if((Math.abs(test[i][left] - test[i][right]) >= threshould) || (Math.abs(test[top][j]-test[bottom][j]) >= threshould)){
        dst[dstOff] = 255;
        dst[dstOff+1] = 255;
        dst[dstOff+2] = 255;
        dst[dstOff+3] = 255;
  ...
  // return output;
  return this.medianFilter(output);
...
Filters.medianFilter = function(pixels){
  ...
  for(var i = 0; i < sh;i++){
    for (var j = 0; j < sw; j++) {
      ...
      var candidate = [];
      candidate[candidate.length] = test[i][j];
      candidate[candidate.length] = test[i][left];
      candidate[candidate.length] = test[i][right];
      candidate[candidate.length] = test[top][left];
      candidate[candidate.length] = test[bottom][left];
      candidate[candidate.length] = test[top][right];
      candidate[candidate.length] = test[top][j];
      candidate[candidate.length] = test[bottom][right];
      candidate[candidate.length] = test[bottom][j];
      // var updown = Math.abs(test[i][left] - test[i][right]);
      // var leftRight = Math.abs(test[top][j] - test[bottom][j]);
      ...
      dst[dstOff] = candidate[4];
      dst[dstOff+1] = candidate[4];
      dst[dstOff+2] = candidate[4];
      dst[dstOff+3] = 255;
    }
  }
  return output;
...
Filters.laplace = function(pixels) {
  ...
  pixels = (this).gaussianBlur(pixels,5);
